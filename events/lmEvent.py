...
#                        await self.gameChannel.send(f'{message.author.mention} You avoided the boss\'s attack')
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        if self.ghost == True and reaction.message.channel.id in [637351107999301633, 276036563866091521]: # This would happen if ghost creation is locked, but message not posted yet
            await reaction.remove(user)
            return await reaction.message.channel.send(f':warning: {user.mention} your reaction was not counted and has been removed. Please try reacting again', delete_after=10)
    ...
    async def on_reaction_remove(self, reaction, user):
        if not self.ghost: return
        if self.ghost == True: return # This would happen if ghost creation is locked, but message not posted yet
        if reaction.message.id != self.ghost.id: return
        if str(reaction) != self.poltergustEmote: return
    ...
    async def percent_test(self, ctx):
        embed = Embed(title="The [un]Scientific Marketplace", colour=0xf6e1b8, description="Professor E. Gadd has been put in a frame by King Boo! All that remains in his lab is a strange sheet with names and prices next to them...\n\n\n")
        ...
        embed.add_field(name=f"8. Strange title plaque | :radio_button: 3", value="I've always been curious what this little bugger was for, but I heard it's valuable! Although I'm sure there are some more... useful items in your journey you should try out first.\n__Exclusive event role on the server. Costs 3 spirit balls__", inline=False)
        embed.add_field(name=f"9. Pretty flower | {self.coin} ???", value='I picked a bunch of these from the strange garden on the roof, but they went missing this morning. Maybe Polterpup just wanted to play with them...\n__???. Single-use__', inline=False)
        embed.add_field(name=f'10. Dark-light device | {self.coin} 15', value='This is one of my favorite inventions! I love the look on a ghost\'s face when they get hit with this.\n__Deals 10% of a ghosts health in bulk damage. Single-use__', inline=False)
        embed.add_field(name=f'11. Exixir | {self.coin} 10', value='An old bottle that I found in a truck. Listen, it won\'t hurt you... or at least I think it is safe to drink.\n__Gives 2x personal dps for 90 minutes. Single-use__', inline=False)
    ...
    async def run_ghost(self, boss=False, magnet=False, summonUser=None):
        ...
        if boss:
            ...
            if magnet: content += f' Strangely, the ghost looks stronger after {summonUser} summoned it... '
        ...
        # Floor multipliers
        if not boss:
            self.maxhp *= 1 + (self.floor * .12)
            value *= 1 + (self.floor * .05)
...
#                    qteTimer -= 1
        ...
        if self.hp > 0:
            ...
            for user, attrs in self.participants.items():
                ...
                if coins <= 0:
                    coins = 1
                msg += f'<@{user}> got {coins} coins' if not 'double' in attrs['flags'] else f'<@{user}> got {coins} coins (Earned double coins)'
    ...
    async def _advance_floor(self, ctx, floor: typing.Optional[int] = 1):
        ...
        embed = Embed(title=f'Current floor is now {self.floor}', description='HP for all ghosts has increased! Ghost hunters, continue the search!\n\nChanges for the new floor:\n\*Prof. E. Gadd lost the stats sheet, so we begin tracking again\n\*Previous levels payed out in golden bones, and reset to 1\n\*Leaderboard command (`!leaderboard`)\n\*Cooldowns for bananas and dark-light device\n\*I hear there are bosses every 4 floors...', color=0xF6E1B8) #pylint: disable=anomalous-backslash-in-string
    ...
    async def _leaderboard(self, ctx):
        ...
        if self.lock:
            return await ctx.send(f'{ctx.author.mention} Command is locked during floor progression. Try again later', delete_after=5)

        rankings, place = await self.calculate_place(ctx.author.id)
        embed = Embed(title='Leaderboard', description=f'Your current placing is **{place[0]}** with **{int(place[1])}** damage points done. Here are the current standings, sorted by total damage dealt overall:', color=0x4A90E2)
    ...
    async def _backpack(self, ctx):
        ...
        if manifest:
            embed = Embed(title=f'Backpack | Level {user["level"]} |{self.coin} {user["coins"]} Coins |:radio_button: {user["spirits"]} Spirits', description='Here are all of your stored items:\n\n' + '\n'.join(manifest))

        else:
            embed = Embed(title=f'Backpack | Level {user["level"]} |{self.coin} {user["coins"]} Coins |:radio_button: {user["spirits"]} Spirits', description='You don\'t have any stored items. You can buy some in <#638872378545274900>')
    ...
    async def _buy(self, ctx, item, quantity: typing.Optional[int] = 1):
        ...
        return await ctx.send(f'{config.redTick} {ctx.author.mention} Professor E. Gadd was put in a frame by King Boo! You cannot use this command right now, go save him instead!', delete_after=10)
        ...
        if quantity <= 0:
            return await ctx.send(f'{config.redTick} {ctx.author.mention} You are trying to buy an invalid amount of items')
        ...
        if item == 8 and 639156722086313984 in [x.id for x in ctx.author.roles]:
            await ctx.message.delete()
            return await ctx.send(f'{config.redTick} {ctx.author.mention} You already have the event role, you cannot buy it again', delete_after=10)
        ...
        if item == 8:
            ...
            if user['spirits'] < self.items[item]['price']:
                return await ctx.send(f'{config.redTick} {ctx.author.mention} Uh oh! You are **{price - user["spirits"]} spirit balls** short of being able to buy {quant} **{self.items[item]["name"]}**', delete_after=10)

            db.update_one({'user': ctx.author.id}, {'$set': {'spirits': user['spirits'] - price}, '$inc': {f'inventory.{item}': 1}})
            return await ctx.send(f'{config.greenTick} {ctx.author.mention} Success! You\'ve purchased {quant} **{self.items[item]["name"]}** for {price} spirits, and you have {user["spirits"] - price} left now. Find and use your item by opening your `!backpack`', delete_after=10)
    ...
    async def _use(self, ctx, item, quantity: typing.Optional[int]):
        ...
        if self.lock:
            return await ctx.send(f'{ctx.author.mention} Command is locked during floor progression. Try again later', delete_after=5)
        if ctx.author.id != 125233822760566784: return await ctx.send(f'{config.redTick} {ctx.author.mention} Professor E. Gadd was put in a frame by King Boo! You cannot use this command right now, go save him instead!', delete_after=10)
        db = mclient.bowser.lmEvent
        try:
            item = int(item)
        ...
        if quantity and quantity <= 0:
            return await ctx.send(f'{config.redTick} {ctx.author.mention} You are trying to use an invalid quantity of items', delete_after=10)
        ...
        while quantity > 0:
            ...
            if item == 1:
                if quantity > 15:
                    await ctx.send(f'{config.redTick} {ctx.author.mention} You cannot use that many **golden bones** at once! Using the max of 15 instead', delete_after=10)
                    quantity = 15
                ...
                gambleItems = [
                    ...
                    {'type': 'negcoin', 'value': 10, 'text': 'Polterpup was so happy to have the bone, it buried it for safe keeping... and knocked you over in the process. Coins spill everywhere from your bag'},
                    {'type': 'poscoin', 'value': 12, 'text': 'Polterpup was so thankful for the bone, it brought you a backpack it found! It\'s filled with coins!'},
                    ...
                    {'type': 'negcoin', 'value': 8, 'text': 'Polterpup is so happy you got a golden bone for it, that it mistook the gold in your pocket for the bone and ran into the mansion!' },
                    ...
                    {'type': 'poscoin', 'value': 7, 'text': 'As soon as the bone left your pocket, a silly little dog with a sock on it\'s head trotted into the room. After taking off the sock, you found some coins inside!'}
                ]
                ...
                if not remove_item(item, check=True): return await ctx.send(f'{config.redTick} {ctx.author.mention} You don\'t have any **{self.items[item]["name"]}**. You can buy some in <#638872378545274900>', delete_after=10)
                if self.ghost: return await ctx.send(f'{config.redTick} {ctx.author.mention} You cannot summon a ghost when one already exists!', delete_after=10)
                ...
                self.bossNum += 1
                return await self.run_ghost(boss=True)#magnet=True, summonUser=ctx.author) # This might be long running, but other command logic has passed so it should be okay
                ...
                return await ctx.send(f'{config.redTick} {ctx.author.mention} This item is disabled at this time', delete_after=10)
                if not remove_item(item): return await ctx.send(f'{config.redTick} {ctx.author.mention} You don\'t have any **{self.items[item]["name"]}**. You can buy some in <#638872378545274900>', delete_after=10)
                ...
                if quantity > 1: return await ctx.send(f'{config.redTick} {ctx.author.mention} You cannot use more than one **{self.items[item]["name"]}** at a time', delete_after=10)
                if not remove_item(item, check=True, count=8): return await ctx.send(f'{ctx.author.mention} The energy courses through you, but it is not enough. Not yet. You need more.', delete_after=10)
